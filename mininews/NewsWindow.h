...
public:
	...
	BOOL Create(LPCTSTR lpClassName, LPCTSTR lpWindowName, int nWidth,
			int nHeight);
	// 设置新闻标题、内容、链接
...
private:
	...
	void DrawNineRect(HDC hdcDest, RECT DestRect, RECT SrcRect, RECT NineRect,
			UINT crTransparent = 0xFF00FF);
	RECT CreateRect(LONG left, LONG top, LONG right, LONG bottom);
	void CreateControl(LPNEWSCONTROL pControl, int nType, int x, int y,
...
public:
	...
	LRESULT AutoClose(UINT message, WPARAM wParam, LPARAM lParam);
};
