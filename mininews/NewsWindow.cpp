...
BOOL CNewsWindow::Create(LPCTSTR lpClassName, LPCTSTR lpWindowName, int nWidth,
		int nHeight) {
	if (IsWindow())
		return FALSE;
	if (!m_hAppSmallIcon) {
		...
		SHFILEINFO shfi;
		SHGetFileInfo(szFileName, FILE_ATTRIBUTE_NORMAL, &shfi,
	...
	HWND hWnd = CreateWindowEx(dwExStyle, lpClassName, lpWindowName, dwStyle, 0,
			0, nWidth, nHeight, NULL, NULL, hInstance, NULL);
	//----------------------------
	if (!hWnd) {
		return FALSE;
	}
...
BOOL CNewsWindow::SetNews(LPCTSTR lpNewsTitle, LPCTSTR lpNewsContent,
		LPCTSTR lpNewsURL) {
	if (!m_nControlCount)
		return FALSE;
...
void CNewsWindow::Show(bool visible) {
	//在桌面右下角显示
	RECT rc;
	...
	SetWindowPos(m_hWnd, NULL, rc.right - nWidth, rc.bottom - nHeight, 0, 0,
...
void CNewsWindow::CreateControl(LPNEWSCONTROL pControl, int nType, int x, int y,
		int nWidth, int nHeight, char * strText) {
...
BOOL CNewsWindow::SetSkin(LPCTSTR lpBitmapName, COLORREF CaptionColor) {
	HBITMAP hBitmap = LoadBitmap((HINSTANCE) GetModuleHandle(NULL),
			lpBitmapName);
	if (!hBitmap)
		return FALSE;
	if (m_hSkinBitmap) {
...
BOOL CNewsWindow::DrawWindow() {
	...
	if (hIcon) {
		DrawIconEx(m_hCacheDC, rcText.left, rcText.top, hIcon, 16, 16, NULL,
				NULL, DI_NORMAL);
		rcText.left += 25;
	}
...
void CNewsWindow::DrawButton(HDC hDC, LPNEWSCONTROL pControl) {
	...
	//-----------------------
	BitBlt(hDC, pControl->x, pControl->y, pControl->nWidth, pControl->nHeight,
			m_hSkinDC, nSrcX, nSrcY, SRCCOPY);
}
void CNewsWindow::DrawStatic(HDC hDC, LPNEWSCONTROL pControl) {
	if (pControl->strText == _T(""))
		return;
	HFONT hFont = NULL;
	UINT uFormat = 0;
	switch (pControl->nType) {
	case NCT_TITLE:
		hFont = m_hBoldFont;
		uFormat = DT_SINGLELINE | DT_VCENTER | DT_CENTER | DT_NOPREFIX
				| DT_WORD_ELLIPSIS;
		break;
	case NCT_CONTENT:
		hFont = m_hFont;
	...
	DrawText(hDC, (pControl->strText).c_str(), -1, &pControl->Rect, uFormat);
...
LRESULT CNewsWindow::OnPaint(UINT message, WPARAM wParam, LPARAM lParam) {
	...
	BitBlt(ps.hdc, ps.rcPaint.left, ps.rcPaint.top, nWidth, nHeight, m_hCacheDC,
			ps.rcPaint.left, ps.rcPaint.top, SRCCOPY);
	EndPaint(m_hWnd, &ps);
	return 0;
}
//画九宫图(目标DC，目标矩形，来源矩形，九宫矩形，透明颜色)
void CNewsWindow::DrawNineRect(HDC hdcDest, RECT DestRect, RECT SrcRect,
		RECT NineRect, UINT crTransparent) {
	...
	xSrc = SrcRect.left;
	ySrc = SrcRect.top;
	::TransparentBlt(hdcDest, x, y, nWidth, nHeight, m_hSkinDC, xSrc, ySrc,
			nWidth, nHeight, crTransparent);
	// 上-------------------------------------;
	...
	::StretchBlt(hdcDest, x, y, nWidth, nHeight, m_hSkinDC, xSrc, ySrc,
...
LRESULT CNewsWindow::OnLButtonUp(UINT message, WPARAM wParam, LPARAM lParam) {
	...
	if (m_nDownIndex != -1) {
		if (m_nDownIndex == nControlIndex) {
			PostMessage(m_hWnd, WM_CONTROLCLICK, (WPARAM) nControlIndex,
					lParam);
		}
		m_nDownIndex = -1;
		DrawWindowEx();
...
LRESULT CNewsWindow::OnControlClick(UINT message, WPARAM wParam,
		LPARAM lParam) {
	int nControlIndex = (int) wParam;
	if (nControlIndex < 0 || nControlIndex >= m_nControlCount)
		return 0;
	//----------------------------
...
//		PostMessage(m_hWnd, WM_CLOSE, NULL, NULL);
		...
		if (m_strURL != _T("")) {
			::ShellExecute(NULL, "Open", m_strURL.c_str(), NULL, NULL, SW_SHOW);
		}
		if (m_bAutoClose) {
...
LRESULT CNewsWindow::OnMessage(HWND hWnd, UINT message, WPARAM wParam,
		LPARAM lParam) {
	switch (message) {
	case WM_ERASEBKGND:
	...
	case WM_DRAWCLIPBOARD:
		...
		if (first != 0) {
			...
			if (OpenClipboard(m_hWnd)) {
				...
				GlobalUnlock(hData);
				CloseClipboard();
				LPCTSTR lpNewsTitle = "abc";
				LPCTSTR lpNewsContent = text;
				LPCTSTR lpNewsURL = ("http://www.baidu.com/");
				this->SetNews(lpNewsTitle, lpNewsContent, lpNewsURL);
				this->Show(true);
//				Sleep(3000);
//				this->Show(false);
			}
...
LRESULT CALLBACK NewsWindow_WndProc(HWND hWnd, UINT message, WPARAM wParam,
		LPARAM lParam) {
